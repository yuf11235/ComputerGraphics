...
public:
	...
	CPoint3D& operator =(const CPoint &pt)
	{
		this->x = pt.x;
		this->y = pt.y;
		this->z = 0.;
	...
	CPoint3D& operator = (const CPoint3D &pt3)
	{
		if(this == &pt3)
		{
			return *this;
	...
	bool operator == (const CPoint3D &ps)
	{
		if(abs(this->x - ps.x) < 0.005 && abs(this->y - ps.y) < 0 && abs(this->z - ps.z) < 0.005)
		{
			return true;
		}
		else
...
public:
	...
	int m_iHideFlag;
	CArray<CPoint3D, CPoint3D> m_point_Array_3D;
	CArray<CPoint3D, CPoint3D> m_point_Array1_3D;

	int m_iIluminationFlag;
	void scanTransfer3D(CDC * pDC, int color_red, int color_green, int color_blue);
	// void scanTransfer3D(CDC * pDC);//for test
	CArray<CPoint, CPoint> m_point_Array_illumination;
	int m_iR;
	int m_iG;
	int m_iB;
...
public:
	...
	int m_iBezierFlag;//for bezier
	void CutPolyLine();

	int m_iTypeFlag;//m_iTypeFlag 1-移动， 2-旋转， 3-x轴镜像， 4-y轴镜像， 5-x轴错移， 6-y轴错移， 7-比例放缩
	double m_Matrix_2D[][3];//齐次矩阵
	// iFlag/: 0-移动， 1-旋转， 2-x轴镜像， 3-y轴镜像， 4-x轴错移， 5-y轴错移， 6-比例放缩
	void GetMatrix(double matrix_2D[][3], int iFlag, double rotateAngle, double x_dis, double y_dis, double dbl_zoom);
	void GetNewPoint();
	void MatrixXMatrix(double matrix0_2D[][3], double matrix1_2D[][3]);
///*
	// for 3d
	// double m_zLength;//拉伸长度
	// double m_dblAngle;//旋转角度
...
protected:
	...
	afx_msg void OnInvisable();
	afx_msg void OnTruesurf();
	afx_msg BOOL OnEraseBkgnd(CDC* pDC);
	afx_msg void OnTimer(UINT nIDEvent);
	afx_msg void OnBesier();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};
