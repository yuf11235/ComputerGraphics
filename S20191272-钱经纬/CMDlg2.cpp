...
void CCMDlg2::OnButton1() 
{
	...
	//this->m_pView2->Invalidate();
	{
		...
		angle = this->m_angle;
		m_pView2->GetMatrix(m_pView2->m_Matrix_2D, 0, 0, (-1)*x_ledge, (-1)*y_ledge, 0);

		m_pView2->GetMatrix(m_Matrix0_2D, 1, (-1)*angle, 0, 0, 0);
		m_pView2->MatrixXMatrix(m_pView2->m_Matrix_2D, m_Matrix0_2D);

		m_pView2->GetMatrix(m_Matrix0_2D, 0, 0, x_ledge, y_ledge, 0);
		m_pView2->MatrixXMatrix(m_pView2->m_Matrix_2D, m_Matrix0_2D);
